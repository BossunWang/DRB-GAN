...
def feature_extraction(conf, test_loader_tgt, label_dict, style_encoding_net):
    style_feature_list = []
    style_label_list = []
    ...
    for it, (style_images, style_img_path) in enumerate(tqdm(test_loader_tgt)):
        ...
        with torch.no_grad():
            style_label_tensor = torch.Tensor([style_label]).long().to(conf.device)
            style_images = style_images.to(conf.device)
            style_prob, style_gamma, style_beta, style_omega = style_encoding_net(style_images, style_label_tensor)
            style_feature = torch.cat([style_gamma, style_beta, style_omega], dim=1).squeeze(0).cpu().numpy()
            style_feature_list.append(style_feature)
            style_label_list.append(style_label)

    style_feature_list = np.array(style_feature_list)
    style_label_list = np.array(style_label_list)
    np.save(os.path.join(conf.cluster_dir, "style_features.npy"), style_feature_list)
    np.save(os.path.join(conf.cluster_dir, "style_labels.npy"), style_label_list)

    return style_feature_list, style_label_list


def UMAP_embedding(feature_path, label_path, save_feature_path, save_umap_path, label_dict):
    features = np.load(feature_path)
    labels = np.load(label_path)
    print('features size:', features.shape)

    reducer = umap.UMAP()
    style_feature_embedded = reducer.fit_transform(features)

    # Training with Labels and Embedding Unlabelled Test Data (Metric Learning with UMAP)
    np.save(save_feature_path, style_feature_embedded)

    colors = ['cyan', 'black', 'green', 'red', 'magenta'
              , 'yellow', 'khaki', 'coral', 'lime', 'peru', 'gold', 'blue', 'pink', 'lavender']
    plt.figure("style UMAP")
    for i, label in enumerate(label_dict):
        plt.scatter(style_feature_embedded[labels == i, 0]
                    , style_feature_embedded[labels == i, 1]
                    , s=10
                    , c=colors[i]
                    , label=label)
    plt.legend(loc='upper left', fontsize=5)
    plt.savefig(save_umap_path)


def TSNE_embedding(feature_path, label_path, save_feature_path, save_tsne_path, label_dict):
    features = np.load(feature_path)
    labels = np.load(label_path)
    print('features size:', features.shape)

    style_feature_embedded = TSNE(n_components=2).fit_transform(features)
    ...
    for i, label in enumerate(label_dict):
        plt.scatter(style_feature_embedded[labels == i, 0]
                    ...
                    , label=label)
    plt.legend(loc='upper left', fontsize=5)
    plt.savefig(save_tsne_path)


def main(conf):
    conf.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # create folders
    if not os.path.exists(conf.cluster_dir):
        os.makedirs(conf.cluster_dir)
    ...
    feature_extraction(conf, test_loader_tgt, label_dict, style_encoding_net)

    UMAP_embedding(os.path.join(conf.cluster_dir, "style_features.npy")
                   , os.path.join(conf.cluster_dir, "style_labels.npy")
                   , os.path.join(conf.cluster_dir, "feature_umap.npy")
                   , os.path.join(conf.cluster_dir, "style_feature_UMAP.jpg")
                   , label_dict)

    TSNE_embedding(os.path.join(conf.cluster_dir, "style_features.npy")
                   , os.path.join(conf.cluster_dir, "style_labels.npy")
                   , os.path.join(conf.cluster_dir, "feature_tsne.npy")
                   , os.path.join(conf.cluster_dir, "style_feature_tsne.jpg")
                   , label_dict)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cluster_dir', type=str, default='clusters/',
                        help='Directory name to save the style cluster with labels')
