...
def calculate_sifid_given_paths(path1, path2, batch_size, cuda, dims, suffix):
    ...
    files1 = sorted(list(path1.rglob('*.%s' % suffix)))

    path2 = pathlib.Path(path2)
    files2 = sorted(list(path2.rglob('*.%s' % suffix)))

    assert len(files1) == len(files2), "file1 size: {}, file2 size: {}".format(len(files1), len(files2))
